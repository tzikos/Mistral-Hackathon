...
const ChatMessage: React.FC<ChatMessageProps> = ({
  ...
  return (
    <div className={`flex mb-4 ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div className={`max-w-[90%] sm:max-w-[85%] md:max-w-xs lg:max-w-md px-3 py-2 sm:px-4 rounded-lg transition-all duration-300 ease-out ${isUser
          ? 'bg-blue-600 text-white ml-auto'
          : `bg-gray-100 text-gray-900 mr-auto ${playbackState?.messageId === message.id && playbackState?.isPlaying
        ...
        <div className="text-sm leading-relaxed">
          {isUser ? (
            ...
            <div className="prose prose-sm max-w-none prose-p:my-2 prose-ul:my-2 prose-ol:my-2 prose-li:my-1 text-left">
              <ReactMarkdown
                components={{
        ...
        {!isUser && message.content.trim() && onPlayTTS && onStopTTS && (
          <div className="flex items-center justify-between mt-2 gap-2 transition-all duration-300 ease-out">
            <div className={`text-xs flex-1 transition-colors duration-200 ${playbackState?.messageId === message.id && playbackState?.isPlaying
